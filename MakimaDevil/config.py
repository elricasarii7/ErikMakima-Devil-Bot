...
def get_user_list(config, key):
    with open('{}/ErzaScarlet/{}'.format(os.getcwd(), config),
              'r') as json_file:
...
class Config(object):
    ...
    OWNER_ID = 7982152132  # If you dont know, run the bot and do /id in your private chat with it, also an integer
    OWNER_USERNAME = "Sohailkhan_anime"
    SUPPORT_CHAT = 'ErzaScarlet_Support'  #Your own group for support, do not add the @
    ...
    KILLERS = get_user_list('elevated_users.json', 'KILLERS')
    SENSEI = get_user_list('elevated_users.json', 'whitelists')
